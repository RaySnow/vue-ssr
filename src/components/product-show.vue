...
<div class="product-show-container">
    <el-row class="product-show" :gutter="10">
        <el-col class="prd-item" v-for="(item) in product_list"
            ...
            <img class="item-avatar" @click="go(item)" :src="item.headPhotoUrl + '?x-oss-process=image/resize,m_pad,w_400,h_400,limit_0'" alt="">
            <div class="prd-item-detail">
                <span class="name">{{item.name}}</span>
                <span class="price" v-if="item.price">
                    {{item.price}}
                    <span v-if="item.priceUnit" class="yuan">元/㎡</span>
                    <span v-else class="yuan">元</span>
                </span>
            </div>
        </el-col>
    </el-row>
            ...
            handleScroll(){
                ...
                if(scroll>0){
                    if(this.$refs.ddd.getBoundingClientRect().top<window.innerHeight && this.fetching) {
                        this.$store.commit(CHANGE_SEARCH_INFO, {key: 'current', value: this.$store.state.product_search_info.current+1})
